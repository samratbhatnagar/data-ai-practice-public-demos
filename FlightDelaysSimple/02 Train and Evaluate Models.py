...
# MAGIC %md There are two reasons for saving the model in this lab. The first is so you can access the trained model later if your cluster restarts for any reason, and also from within another notebook. Secondly, we will need to make the model available externally so we can deploy it using Azure Machine Learning. Save the model to DBFS so it can be accessed across any clusters in the Databricks Workspace.
# MAGIC 
# MAGIC NOTE: Save the model in the root of DBFS as this is where Spark Pipelines will look by default, and changing to any other subfolder will break the convention used by Azure Machine Learning services scoring scripts that are test locally.  
